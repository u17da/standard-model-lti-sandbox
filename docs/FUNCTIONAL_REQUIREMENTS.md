...
5. **Tool API**: Launch情報を検証し、Firebaseカスタムトークンを発行してシームレスにログイン。
6. **Portal (Browser)**: ホームページ（`/`）へリダイレクトされ、必要に応じてオンボーディング画面を表示。
...
- **Authorization Request 検証 (Phase 2)**
  ...
  - **リダイレクトURIの柔軟な検証**: `http://localhost:3000` などの開発用オリジンへの遷移を許容。
...
- **結果表示と自動ログイン**
  - 受信したClaimsの詳細表示。
  - **Firebase連携**: LTIアイデンティティに基づくカスタムトークン生成と自動ログイン（オートログイン）。
  - **オンボーディングフロー**: 新規ユーザーまたはプロファイル未設定ユーザーに対する `OnboardingProfileDialog` の自動表示。
  - ホームページ（`/`）への自動リダイレクト。
...
- **最適化**: 読み取りコスト削減のため、デルタフェッチ（差分取得）によるポーリングを実装。
